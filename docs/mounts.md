...
- `rider-view.hide-flying-mount` sends rider-only invisibility metadata after the camera reaches `hide-at-pitch`; `show-at-pitch` is a lower return threshold that prevents flicker. Other players continue to see the mount, and the client keeps the vehicle relationship. With no saved rider preference, ordinary forms remain visible; automatic hiding defaults on only at effective appearance scale 2 or above. The same resolved default is used by summoning, live tuning and the tuning button, while an explicit player preference always wins.
