...
class MockGuiItemFactory : GuiItemFactory {
    ...
    override fun create(item: ItemStack): GuiItem = createMockGuiItem(item, null)

    override fun create(
        item: ItemStack,
        action: Consumer<InventoryClickEvent>,
    ): GuiItem = createMockGuiItem(item, action)

    override fun create(material: Material): GuiItem = createMockGuiItem(ItemStackMock(material, 1), null)

    override fun create(
        material: Material,
        action: Consumer<InventoryClickEvent>,
    ): GuiItem = createMockGuiItem(ItemStackMock(material, 1), action)

    private fun createMockGuiItem(
        item: ItemStack,
        action: Consumer<InventoryClickEvent>?,
    ): GuiItem {
        ...
        doAnswer { invocation ->
            ...
            null
        }.whenever(mockGuiItem).isVisible = any()
