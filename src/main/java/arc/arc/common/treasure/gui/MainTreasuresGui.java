package arc.arc.common.treasure.gui;
...
public class MainTreasuresGui extends ChestGui implements Inputable {
    ...
    public MainTreasuresGui(Player player) {
        super(rows, TextHolder.deserialize(TextUtil.toLegacy(config.string("main.title"))));
        this.player = player;
    ...
    public void setParameter(int n, String s) {
        if (n == 0) {
            ...
            GuiUtils.constructAndShowAsync(() -> new PoolGui( player, existing), player);
        }
    }
