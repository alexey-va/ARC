    ...
    private Player getEnteredPlayer(Collection<Player> nearby) {
        if (player.hasPermission("arc.portal.tp-other")) {
            for (Player player : nearby) {
                if (inPortal(player, centerBlock.getLocation()) && player.hasPermission("arc.portal.tp-by-other"))
                    return player;
            }
        } else if (inPortal(player, centerBlock.getRelative(0, 1, 0).getLocation())) {
            return player;
        }
        return null;
    }
    ...
    private void executeAction(Player player) {
        ...
        if (actionType == COMMAND) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    player.getWorld().playSound(player.getLocation(), ENTITY_ENDERMAN_TELEPORT, 1f, 1f);
                    HookRegistry.huskHomesHook.teleport(portalData.getHuskTeleport());
                }
            }.runTask(ARC.plugin);
