...
Без аргументов игрок сначала выбирает источник: «Банк», «Кузница»,
«Гильдия» или отдельный общий список. В каталоге остаются только заказы,
которые уже принимают ресурсы; будущие, закрытые и исчерпанные позиции скрыты.
Недельный приём заканчивается после воскресенья: новое окно начинается ровно
в понедельник в 00:00 по часовому поясу `Europe/Moscow`.
Просмотр условий доступен без ресурсов и вне спавна; NPC открывают сразу
свой каталог через `open <группа>`. `status` сохраняет
публичную текстовую сводку, а консоль без аргументов получает именно её.
Сдача по-прежнему требует клика по настроенному NPC и нахождения рядом с ним:
просмотр другой вкладки не выдаёт разрешение и не сбрасывает разрешение своей.

В карточке есть выбор минимальной партии, стака (с учётом размера стака
предмета и доступных границ) и максимума. Итоговая сумма рассчитывается за
партию, вне приёмщика это предварительный расчёт. Успешная сдача показывает
реальный результат, личный вклад, остаток и следующую подходящую позицию.
Один заказ можно отслеживать как цель сбора; это не резерв и не обещание цены.
Обновление книги не гарантирует повтор того же ресурса.
...
`/arc auctionpedestal add` ставит витрину аукциона на верхнюю поверхность
твёрдого блока под прицелом (до 6 блоков, в постоянно загруженной области
спавна Origin). Право `arc.origin.auction.admin` по умолчанию доступно OP.
ПКМ администратора по витрине открывает native dialog с текущим лотом и
удалением через подтверждение. Удаляется только витрина, сам лот сохраняется.
Обычный игрок открывает лот напрямую; предмет дела расследования в руке
имеет приоритет над нажатием по витрине.

Полный список, в том числе пустой, хранится в runtime-файле
`plugins/ARC/data/auction-showcase-pedestals.json`. Его отсутствие означает
стартовый список из `origin-spawn.yml`; этот data-файл не разворачивают из Git.
...
## /arc audit reset-period

Начинает новый общий период продуктовой телеметрии и экономических сводок.
Вызывается на основном ARC-сервере; нужны `arc.audit` и `arc.audit.reset` (OP по умолчанию).

1. `/arc audit reset-period` показывает текущую границу и команду подтверждения.
2. `/arc audit reset-period confirm <generation>` начинает период, только если поколение ещё актуально.
3. `/arc audit reset-period status` читает общую границу из Redis.

Старые продуктовые файлы остаются рядом как архивы. Новые записи идут в
`product-interest-period-<timestamp>.json`; рестарт продолжает этот файл.
Экономические сводки используют максимум запрошенной даты и общей границы.
SQL-аудит остаётся доступен через просмотр игрока и обычные правила retention;
балансы, награды, лимиты и журналы защиты от повторных выплат не изменяются.
Это не `/arc audit clearall`, и Prometheus-история счётчиков не стирается.

Узлы применяют границу при старте и опрашивают её каждые 5 секунд. Продуктовая
доставка best effort: события между общей границей и локальным применением
могут остаться только в предыдущем архиве. Поле `measurementReset` отчёта
показывает локальное применение, начальный промежуток и доступность control.
Новый период начинает новые наблюдения и когорты; старый first-join не переносится.
Пересекающие границу агрегированные денежные записи исключаются из точной сводки
и отражаются её `windowBoundary`; сутки границы защищены от последующей дневной компактизации.
После ошибки подтверждения сначала проверяйте status: запись общей границы могла
завершиться, даже если локальное применение ещё не удалось.
