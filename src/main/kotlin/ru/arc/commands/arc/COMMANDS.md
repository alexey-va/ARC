...
Показывает публичную доску Minecraft-native заказов: точный предмет, оставшуюся
квоту, цену за единицу и срок окна. Права не требуются.

На этапе калибровки каталог пуст, а `submit` остаётся безопасной заглушкой:
предметы не забираются и деньги не выдаются. В коде уже есть выключенный
вертикальный срез для обычных vanilla-предметов без meta/NBT: journal одновременно
резервирует quantity/budget, затем перед каждым удалением, платежом и возвратом
долговечно записывается intent, а RedisEconomy balance сверяется в точных minor
units. Неоднозначный результат никогда не повторяется автоматически.

Режим `enforce` в YAML сам по себе не разблокирует операции: compile-time runtime
gate остаётся закрытым до retention terminal-записей, authenticated operator
reconciliation и production crash smoke.

```bash
/arc contracts
/arc contracts status
/arc contracts submit road_stone 64
```
...
служит `survival`; обычный запрос всегда запускает RTP, даже после перехода с
другого сервера. Флаг `--only-if-first` сохраняет поведение порталов: RTP
запускается только при первом посещении мира, а повторный вход возвращает в
этот мир без RTP. После межсерверного перехода ARC повторно накладывает
портальную слепоту уже на целевом backend.
