...
На этапе калибровки каталог пуст, а `submit` работает только как безопасная
заглушка: предметы не забираются и деньги не выдаются. Режим `enforce` в YAML
сам по себе не разблокирует операции до готовности атомарного журнала выплат.
