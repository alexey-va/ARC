...
Переход между inventory-меню выполняется обычным `ArcMenus.open`: общий runtime
завершает прежнюю сессию и сразу показывает следующую, без промежуточного
закрытия окна. Не вызывайте `player.closeInventory()` перед следующим GUI и не
добавляйте задержку между закрытием и открытием: это возвращает клиент в игру
и сбрасывает положение курсора. Для обновления текущей сессии используйте
`refresh()`/`requestRefresh()`, для её встроенной пагинации — `nextPage()` и
`previousPage()`. Настоящее закрытие, Escape и reload завершают окно штатно.
...
Для хранилищ используется `ArcMenus.openStorage` и общий вариант
`PaperCloudStorage` из `arc-core-paper-menu`. Он открывает нативный сундук без
служебных меток InventoryFramework. ArcStore и персональный лут передают
отделённый снимок слотов и атомарный `compareAndSet`; ограничения предметов и
сохранение остаются в `StoreData`/`CustomLootData` и их репозиториях.

ЛКМ берёт стопку на курсор, ПКМ — половину. В собственном инвентаре работают
обычные клики и перетаскивание, поэтому игрок выбирает слот сам. Shift переносит
столько, сколько помещается; остаток остаётся в источнике. При устаревшем
снимке содержимое обновляется без выдачи. Hotbar swap, drop, creative clone,
double-click и drag, затрагивающие верхнее хранилище, блокируются. Закрытие,
выход и reload завершают сессию; курсор обрабатывает нативный lifecycle Paper.

ArcStore принимает разрешённые предметы. Лут данжей разрешает только изъятие.
...
открытии. До четырёх свободных ячеек после исходного списка занимают
декоративные паутина, булыжник, земля и палка. Они не записываются в добычу и
не выдаются игроку. Остальные ячейки пустые, без панелей.

Маунты имеют специализированные контроллеры. Они получают размеры,
управляющие слоты и регионы из того же валидированного `ArcMenus`-каталога;
материалы, name и lore остаются в подробных feature-конфигах.
