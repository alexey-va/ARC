...
/** Legacy suffixes and full MiniMessage templates; mirrored by ProxyARC at the wire boundary. */
internal object CustomJoinMessage {
    ...
    private val miniMessage = MiniMessage.builder().tags(net.kyori.adventure.text.minimessage.tag.resolver.TagResolver.resolver(
        StandardTags.color(), StandardTags.decorations(), StandardTags.reset(), StandardTags.gradient(), StandardTags.rainbow(), StandardTags.font(),
    )).build()

    fun normalize(raw: String): String {
        require(raw.none { it.isISOControl() || Character.getType(it) == Character.FORMAT.toInt() || it in setOf('&', '§', '\\') }) {
            "Custom message must be a single line"
        }
        val text = raw.trim()
        require(text.isNotBlank() && text.length <= MAX_LENGTH) { "Invalid custom message length" }
        if (PLAYER !in text) {
            require(text.length <= 120 && text.none { it in setOf('<', '>', '%', '#') }) { "Invalid legacy suffix" }
            return text
        }
        require(text.windowed(PLAYER.length).count { it == PLAYER } == 1 && '%' !in text.replace(PLAYER, "")) { "Use exactly one player placeholder" }
        require(tags.findAll(text).all { tag.matches(it.groupValues[1]) }) { "Unsupported formatting tag" }
        val literal = tags.replace(text, "")
        require('<' !in literal && '>' !in literal) { "Incomplete formatting tag" }
        val rendered = render(text, "Player1234567890")
        val plain = PlainTextComponentSerializer.plainText().serialize(rendered)
        require(plain.length <= MAX_VISIBLE_LENGTH && '<' !in plain && '>' !in plain) { "Invalid rendered message" }
        return text
    }
